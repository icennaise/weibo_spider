...
def getMidList():
    db = pymysql.connect(host=host, user=user, password=password, db="sinaweibo")
    cursor = db.cursor()
    midList = []
    sql = "select mid from card"
    try:
        cursor.execute(sql)
        db.commit()
...
def parseTimeStr(timeStr):
    y, m, d, h = timeStr.split('-')
    return datetime.datetime(int(y), int(m), int(d), int(h))
